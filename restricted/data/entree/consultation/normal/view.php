...
<?php
  ...
  $sql="select * from vignette inner join fournisseur_client on vignette.frID=fournisseur_client.frID inner join categorie on vignette.categorieID=categorie.categorieID where vignetteID ='$id'and programmeID=1";

  $records=mysqli_query($connect, $sql);
  function mt1($connect)
  {
    if(isset($_GET['id']))
    {
    $id = intval($_GET['id']);
    ...
    $output = '';
    $sql = "select * from vignette where categorieID='$id'";
    $result = mysqli_query($connect,$sql);
    while($row=mysqli_fetch_array($result))
    {
      $output .='<option value="'.$row["vignetteID"].'">'.$row["valeurE"].'</option>';
    }
    return $output;
  }
  function mt2($connect)
  {
    if(isset($_GET['id']))
    {
    $id = intval($_GET['id']);
    ...
    while($row=mysqli_fetch_array($result))
    {
      $output .='<option value="'.$row["vignetteID"].'">'.$row["valeurEE"].'</option>';
    }
    return $output;
  }
  ...
  while ($sorties=mysqli_fetch_assoc($records)) {
    $num = "<td>".$sorties['num_conv']."</td>";
    $acq = "<td>".$sorties['nom_prenom']."</td>";
    $dire = "<td>".$sorties['direction']."</td>";
    $date ="<td>".$sorties['date']."</td>";
    $des = "<td>".$sorties['type']."</td>";
    $mont = "<td>".number_format($sorties['valeurE'], 2)."</td>";
    $mont2 = "<td>".number_format($sorties['valeurEE'], 2)."</td>";
...
<HTML>
  ...
  <li style="float:right"><a class="active" href="about.asp">About</a></li>
    ...
    <h2 align="center"class="h2">BON D'ENTRE VIGNETTES <br><?= $type; ?></h2>
    <table class="n1">
      <tr>
