...
?>
  ...
  $sum = "select sum(valeurE) as Total1 from vignette where not libelle='stock initial' and categorieID=$categorieID and programmeID=1 and exercice='$date' and direction LIKE '%$nm%'";
  $tot = mysqli_query($connect, $sum);
  $sum1 = "SELECT SUM(valeurS) AS Total2 from vignette where categorieID='$categorieID' and programmeID=1 and exercice='$date' and direction LIKE '%$nm%'";
